  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_BASE_URL}/api/interview-experiences`,
        {
  ...
  const getAppliedJobs = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/api/applications/${loginJsData._id}`,
  ...
  const handleWithdrawApplication = async (applicationId) => {
    try {
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/api/applications/${applicationId}`
      );
